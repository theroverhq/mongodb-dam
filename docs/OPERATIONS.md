...
Outpost `/ready` verifies access to its spool. S3 or HTTP destination availability does not make it unready: temporary destination loss is absorbed by the PVC and exposed through spool and delivery-failure metrics.
...
Observer writes a content-addressed, fsync-backed file before sending a batch to Outpost. Outpost acknowledges Observer only after writing its own fsync-backed copy. In S3 mode, Outpost deletes that copy only after `PutObject` succeeds; all S3 errors retain the spool item for retry. The deterministic object key means a retry targets the same logical key. In HTTP mode, Outpost deletes the copy after a `2xx` or duplicate `409`; a permanent `4xx` moves it to the spool's `quarantine/` directory for operator review.
...
For S3, the AWS SDK credential chain is used. The chart can expose credentials from `destination.s3.credentialsSecretName`, but EKS Pod Identity or IRSA is preferred outside this demo. Restart Outpost after rotating a static credential Secret. Bucket and prefix are supplied through `OUTPOST_S3_BUCKET` and `OUTPOST_S3_PREFIX`; object format and permissions are in [S3_EXPORT_CONTRACT.md](S3_EXPORT_CONTRACT.md).

For the optional HTTP mode, update the `bearer-token` key in the configured Kubernetes Secret, then restart Outpost. The endpoint is posted exactly as configured and redirects are not followed. A private endpoint CA may be supplied through `destination.caSecretName`.
