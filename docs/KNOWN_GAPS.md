...
- A truncated compressed frame cannot be decoded safely. Compression destroys field locality. The frame is counted as a parse gap and no partial query content is exported.
- Data beyond the 1 KiB bounded capture prefix is unavailable. With `OBSERVER_CAPTURE_QUERY_CONTENT=true`, `details.query` is therefore emitted only when the complete `find`, `aggregate`, `insert`, `update`, or `delete` command document is available; larger commands keep their activity metadata but omit the query. For example, a large write can report `insert` and its collection while `$db` may be absent if a driver places it after a large inline document.
